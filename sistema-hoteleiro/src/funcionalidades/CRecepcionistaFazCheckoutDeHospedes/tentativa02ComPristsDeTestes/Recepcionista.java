...
public class Recepcionista extends Thread {
    ...
    private void alocarGruposNoQuarto(Quarto quarto) {
        synchronized (quarto) {
...
//        quarto.devolverChaveNaRecepcao(); // Verifica se a chave deve ser devolvida na recepção
    }
    ...
    private void temporizarEstadia(GrupoHospedes grupo, Quarto quarto) {
        ...
        new Thread(() -> {
            try {
                Thread.sleep(2000); // Tempo de estadia de 2 segundos (alterável conforme necessário)
...
//        synchronized (quarto) {
//            if (quarto.getHospedes().contains(grupo)) {
//                quarto.getHospedes().remove(grupo);
//                quarto.setDisponivel(true);
//                System.out.println("Recepcionista " + nome + " realizou o check-out do " + grupo.getNome() + " do quarto " + quarto.getNumeroQuarto());
//            }
//        }
//    }
...
//        System.out.println("Hóspedes atualmente hospedados no quarto:"+quarto.getNumeroQuarto());
...
//        }
        ...
        synchronized (quarto) {
            if (quarto.getGrupos().contains(grupo)) {
                System.out.println("tst0 - "+quarto.getGrupos().size()); // numero de grupo dentro do quarto ... se o quarto já estiver alugado sempre deve ser 1
                System.out.println("(tst1-quarto"+quarto.getNumeroQuarto()+")Nome do grupo: "+quarto.getGrupos().get(0).getNome()); // nome do grupo
                System.out.println("(tst2-quarto"+quarto.getNumeroQuarto()+")Numero de hospedes: "+quarto.getGrupos().get(0).getHospedes().size()); // qtd de hospedes no grupo
                System.out.println("(tst3-quarto"+quarto.getNumeroQuarto()+")Nome do 1o hospede do "+quarto.getGrupos().get(0).getNome()+": "+quarto.getGrupos().get(0).getHospedes().get(0).getNome()); // nome do primeiro hospede do grupo
                quarto.removerGrupoDeHospedes(grupo);
                System.out.println("Numero de hospedes(pos checkOut): "+quarto.getGrupos().size());
                quarto.setDisponivel(true);
                System.out.println("Recepcionista " + nome + " realizou o check-out do " + grupo.getNome() + " do quarto " + quarto.getNumeroQuarto());
    ...
    /*  colocar depois o comentário -> 2ºNvl-Paralelismo: Dentro do método run() da
        classe Recepcionista, as operações de alocação de quartos para os grupos de
        hóspedes são realizadas de forma independente. Cada thread de Recepcionista
        procura por quartos disponíveis e aloca grupos de hóspedes, sem depender das
        outras threads. Este é o segundo nível de paralelismo, onde as threads de
        Recepcionista operam de forma independente, executando suas tarefas em paralelo.
    */
}
